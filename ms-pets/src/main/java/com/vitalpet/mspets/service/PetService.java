...
public class PetService {
    ...
    public PetResponseDTO create(PetRequestDTO dto) {
        Species species = speciesRepository.findById(dto.getSpeciesId())
                .orElseThrow(() -> new ResourceNotFoundException("Error: La especie con ID " + dto.getSpeciesId() + " no existe."));
    ...
    public PetResponseDTO update(Long id, PetRequestDTO dto) {
        Pet existing = petRepository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("Error: La mascota con ID " + id + " no existe."));
